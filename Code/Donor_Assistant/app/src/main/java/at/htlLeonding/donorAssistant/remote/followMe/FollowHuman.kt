    ...
    private fun startFollowingHuman(useStraightLines: Boolean) {
        ...
        goToFuture = humanToFollow.async().headFrame.andThenCompose { headFrame ->
            if (lookAtFuture == null) {
                lookAtFuture =
                    LookAtBuilder.with(qiContext).withFrame(headFrame).buildAsync().andThenCompose {
            ...
            GoToBuilder.with(qiContext).withFrame(humanToFollow.headFrame)
                .withPathPlanningPolicy(pathPlanningPolicy).buildAsync().andThenCompose {
